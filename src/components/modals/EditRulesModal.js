...
const UpdateRuleModal = ({ isOpen, onRequestClose, rule, onUpdateRule, conditions }) => {
  ...
  useEffect(() => {
    if (rule) {
      setIConditionId(rule.i_condition_id || '');
      setAFConditionId(rule.af_condition_id || '');
      setKGConditionId(rule.kg_condition_id || '');
      setTSConditionId(rule.ts_condition_id || '');
      setHConditionId(rule.h_condition_id || '');
      setResultId(rule.result_id || '');
      setCF(rule.cf || '');
    }
  }, [rule]);
  ...
  return (
    <Modal
      ...
      contentLabel="Update Rule Modal"
